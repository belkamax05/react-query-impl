...
const View = () => {
  ...
  const handleRemove: MouseEventHandler<HTMLButtonElement> = useCallback((event) => {
  ...
  return (
    <Box>
      ...
      <Box>
        {todos?.map((todo) => (<Box key={todo.id}>
          <span>{todo.title}</span>
          <Button data-id={todo.id} onClick={handleRemove} size="small" variant="contained">Delete</Button>
        </Box>))}
      </Box>
    </Box>
  );
